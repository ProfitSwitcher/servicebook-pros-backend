 ...
 * Analytics Routes
 *
 * These endpoints provide aggregated metrics and stub predictive analytics.  They are intended
 * as a foundation for Phase 4 (AI & Advanced Analytics).  In a real implementation,
 * complex ML models and historical data analysis would be incorporated.  For now,
 * we compute simple summaries directly from the database and return placeholder predictions.
 */

// GET /analytics/summary
// Returns basic aggregated metrics across customers, jobs and invoices.
app.get('/analytics/summary', authenticateToken, async (req, res) => {
  try {
    const customersCountRes = await pool.query('SELECT COUNT(*)::INTEGER AS count FROM customers');
    const jobsCountRes = await pool.query('SELECT COUNT(*)::INTEGER AS count FROM jobs');
    const completedJobsCountRes = await pool.query("SELECT COUNT(*)::INTEGER AS count FROM jobs WHERE status = 'completed'");
    const invoicesTotalRes = await pool.query('SELECT COALESCE(SUM(amount),0)::NUMERIC(12,2) AS total FROM invoices');
    const avgDurationRes = await pool.query(
      `SELECT AVG(EXTRACT(EPOCH FROM (end_time - start_time))) AS avg_seconds
       FROM jobs
       WHERE end_time IS NOT NULL AND start_time IS NOT NULL`
    );
    const customers_count = customersCountRes.rows[0].count;
    const jobs_count = jobsCountRes.rows[0].count;
    const completed_jobs_count = completedJobsCountRes.rows[0].count;
    const total_invoice_amount = parseFloat(invoicesTotalRes.rows[0].total);
    const average_job_duration_seconds = avgDurationRes.rows[0].avg_seconds ? parseFloat(avgDurationRes.rows[0].avg_seconds) : null;
    res.json({
      customers_count,
      jobs_count,
...
// Returns metrics specific to a customer: job count, total invoiced and average job duration.
app.get('/analytics/customer/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
    // verify customer exists
    const custRes = await pool.query('SELECT id FROM customers WHERE id = $1', [id]);
    if (custRes.rowCount === 0) {
      return res.status(404).json({ error: 'Customer not found' });
    }
    const jobCountRes = await pool.query('SELECT COUNT(*)::INTEGER AS count FROM jobs WHERE customer_id = $1', [id]);
    const invoiceTotalRes = await pool.query('SELECT COALESCE(SUM(amount),0)::NUMERIC(12,2) AS total FROM invoices WHERE job_id IN (SELECT id FROM jobs WHERE customer_id = $1)', [id]);
    const avgDurationRes = await pool.query(
      `SELECT AVG(EXTRACT(EPOCH FROM (end_time - start_time))) AS avg_seconds
       FROM jobs
       WHERE customer_id = $1 AND end_time IS NOT NULL AND start_time IS NOT NULL`,
      [id]
    );
    const metrics = {
      customer_id: parseInt(id),
      jobs_count: jobCountRes.rows[0].count,
      total_invoice_amount: parseFloat(invoiceTotalRes.rows[0].total),
      average_job_duration_seconds: avgDurationRes.rows[0].avg_seconds ? parseFloat(avgDurationRes.rows[0].avg_seconds) : null,
    };
    res.json(metrics);
  } catch (err) {
    console.error(err);
...
// Returns simple, placeholder predictions.  In a full implementation, this would call a
// trained ML model and return meaningful forecasts.
app.get('/analytics/predict', authenticateToken, async (req, res) => {
  try {
    // Example: predict next month's revenue based on average monthly invoice total.
    // Compute total invoice per month for the last 3 months; if none, use total.  Here we use a simple heuristic.
    const revenueRes = await pool.query(
      `SELECT DATE_TRUNC('month', issued_at) AS month, SUM(amount) AS total
       FROM invoices
       GROUP BY month
       ORDER BY month DESC
       LIMIT 3`
    );
    const revenues = revenueRes.rows.map((row) => parseFloat(row.total));
    const avgRevenue = revenues.length > 0 ? revenues.reduce((a,b) => a + b, 0) / revenues.length : 0;
    const predictedNextMonthRevenue = avgRevenue * 1.1; // simple 10% growth assumption
    res.json({
      description: 'Predicted next month revenue using simple growth heuristic',
      predicted_next_month_revenue: parseFloat(predictedNextMonthRevenue.toFixed(2)),
      average_recent_monthly_revenue: parseFloat(avgRevenue.toFixed(2)),
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
